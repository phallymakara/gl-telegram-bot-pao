/**
 * @file StockAlertsView.tsx
 * @description Sub-component rendering low stock threshold alerts list, statistics, and action controls.
 */

import { AlertCircle, Package, PauseCircle, Pencil, Plus, Trash2 } from "lucide-react";
...
/**
 * View component for low stock alerts.
 */
          ...
          <table className="w-full text-sm min-w-[1000px]">
            <thead className="sticky top-0 z-10">
              <tr className="text-left text-xs text-slate-400 uppercase tracking-wide border-b border-slate-200 bg-slate-50">
                {["#", "Title", "Message", "Premium", "Created", "Status", "Actions"].map((h) => (
                ...
                <tr key={a.id} className="border-b border-slate-50 align-top hover:bg-slate-50/60">
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-1">
                      ...
                      <IconBtn title="Delete" tone="danger" onClick={() => deleteAlert(a.id)}>
                        <Trash2 size={15} />
                      </IconBtn>
                    </div>
                  </td>
