/**
 * @file PromoDiscountsView.tsx
 * @description Sub-component rendering discount promotions list, campaign status, and promotion controls.
 */

import { AlertCircle, Calendar, Pencil, Plus, Tag, Trash2 } from "lucide-react";
import Card from "../../components/Card";
import IconBtn from "../../components/IconBtn";
import StatCard from "../../components/StatCard";
import StatusBadge from "../../components/StatusBadge";
import { AlertData, toNumber } from "../../api";

interface PromoDiscountsViewProps {
  alerts: AlertData[];
...
/**
 * View component for discount promotion rules.
 */
export default function PromoDiscountsView({ alerts, notify, deleteAlert }: PromoDiscountsViewProps) {
  return (
    <div className="flex-1 pt-4 px-4 pb-2 sm:pt-4 sm:px-8 sm:pb-2 min-w-0 overflow-hidden w-full flex flex-col space-y-3 min-h-0">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 flex-shrink-0">
        ...
        <StatCard
          ...
          value={
            alerts.filter((a) => a.start_at && new Date(a.start_at) > new Date()).length
          }
                ...
                <tr key={p.id} className="border-b border-slate-50 align-top hover:bg-slate-50/60">
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-1">
                      ...
                      <IconBtn title="Delete" tone="danger" onClick={() => deleteAlert(p.id)}>
                        <Trash2 size={15} />
                      </IconBtn>
                    </div>
                  </td>
