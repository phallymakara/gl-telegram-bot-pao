  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 w-full items-start">
      <div className="md:col-span-1 md:sticky md:top-4 self-start flex flex-row overflow-x-auto flex-nowrap gap-1.5 md:flex-col scrollbar-none min-w-0 border-b md:border-b-0 border-slate-200/60 pb-3 md:pb-0 z-10 bg-slate-50/90 backdrop-blur-xs md:bg-transparent">
        <div className="hidden md:block px-3 mb-2 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Settings Menu</div>
        {tabs.map((tab) => {
          const Icon = tab.icon;
          const isActive = activeTab === tab.id;
          return (
            <button key={tab.id} onClick={() => scrollToSection(tab.id)}
              className={`shrink-0 whitespace-nowrap text-left px-3.5 py-2.5 md:py-3 rounded-lg transition-all duration-150 flex items-center gap-2.5 relative focus:outline-none cursor-pointer ${isActive ? "bg-indigo-50 md:bg-slate-100 text-indigo-700 md:text-slate-900 font-semibold shadow-xs" : "text-slate-600 hover:bg-slate-50/80 hover:text-slate-900"}`}>
              {isActive && <span className="hidden md:block absolute left-0 top-3 bottom-3 w-1 rounded-r bg-indigo-600" />}
