...
import { usersSeed, UserItem } from "../data/mockData";
...
export default function UsersPage({ notify }: UsersPageProps) {
  ...
  const filtered = users.filter((u) => {
    ...
    const ms = statusF === "All Statuses" || u.status === statusF;
    return mq && mr && ms;
  });
    ...
    <div className="flex-1 pt-4 px-4 pb-2 sm:pt-4 sm:px-8 sm:pb-2 min-w-0 overflow-hidden w-full flex flex-col space-y-3 min-h-0">
      ...
      <Card className="flex-1 flex flex-col min-h-0 overflow-hidden">
        <div className="p-5 border-b border-slate-100 flex flex-col sm:flex-row gap-3 flex-shrink-0">
          ...
          />
          <button
            onClick={() => notify("Invite sent")}
            className="flex items-center gap-2 text-sm px-4 py-2.5 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 font-medium whitespace-nowrap"
          >
          ...
          <table className="w-full text-sm">
            <thead className="sticky top-0 z-10">
              <tr className="text-left text-xs text-slate-400 uppercase tracking-wide border-b border-slate-200 bg-slate-50">
                {["#", "Name", "USER ID", "Email", "Password", "Role", "Status", "Last Login", "Actions"].map(
                  (h) => (
            ...
            <tbody>
              {filtered.map((u, i) => (
                <tr key={u.id} className="border-b border-slate-50 hover:bg-slate-50/60">
                  ...
                  <td className="px-5 py-3.5">
                    <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${roleTint[u.role]}`}>
                      {u.role}
                    </span>
                  </td>
                  <td className="px-5 py-3.5">
                  ...
                  <td className="px-5 py-3.5 text-slate-500 whitespace-nowrap">
                    {(() => {
                      const parts = u.last.split(" ");
                      if (parts.length >= 3) {
                        return (
                          <>
                            <div className="font-medium">{parts.slice(0, 3).join(" ")}</div>
                            <div className="text-xs text-slate-400 mt-0.5">{parts.slice(3).join(" ")}</div>
                          </>
                        );
                      }
                      return u.last;
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-1">
                      ...
                      <IconBtn
                        ...
                        onClick={() => {
                          if (u.you) {
                            notify("You can't delete your own account");
                            return;
                          }
