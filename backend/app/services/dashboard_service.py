...
def calculate_dashboard_stats(db: Session, target_date: str = "") -> DashboardStats:
    ...
    gold_in_local = float(db.query(func.coalesce(func.sum(PurchaseOrder.quantity), 0)).filter(
    ...
    gold_out_phone = float(db.query(func.coalesce(func.sum(Order.quantity), 0)).filter(
        Order.transaction_type == "SELL",
        Order.channel == "PHONE"
    ).scalar())

    gold_out_walkin = float(db.query(func.coalesce(func.sum(Order.quantity), 0)).filter(
        Order.transaction_type == "SELL",
